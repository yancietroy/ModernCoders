...
$nav_breadcrumbs = [
	...
	["Election List", "", "bi-clipboard-data-fill"],
];
...
<body>
					...
					<div class="row g-0 justify-content-center ">
						<div class="table-responsive ms-2">
							<table id='elections-table' class='py-3 display nowrap w-100 ms-0 stud'>
								<thead>
									<tr>
									<th class="desktop">Election ID</th>
								...
								<tbody>
									<?php
									$sql = "SELECT * FROM tb_elections ORDER BY ELECTION_ID DESC";
									...
									?>
											...
											<td><?= $row['ELECTION_ID'] ?></td>
												...
												<td>
													...
													<button type='button' class='btn btn-secondary btn-sm deletebtn' id="<?= $row['ELECTION_ID'] ?>"> <i class=' bi bi-archive-fill'></i> </button>
												</td>
	...
	<script>
		$(document).on('click', '.viewbtn', function() {
			...
			location.href = "admin-election-view.php?election_id=" + ELECTION_ID;
		});
		$(document).on('click', '.deletebtn', function() {
			var ELECTION_ID = $(this).attr("id");
			$('#delete_id').val(ELECTION_ID);
			$('#deletemodal').modal('show');
		});
		$(document).on('click', '.resultbtn', function() {
			var ELECTION_ID = $(this).attr("id");
			location.href = "admin-election-result.php?id=" + ELECTION_ID;
		});
	</script>
	<!-- Datatable -->
