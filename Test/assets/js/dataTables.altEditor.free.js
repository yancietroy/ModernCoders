 ...
 *  Modified by: Kasper Olesen (https://github.com/KasperOlesen), Luca Vercelli
 * (https://github.com/luca-vercelli), Zack Hable (www.cobaltdevteam.com)
...
var altEditor =
  ...
  this.c = $.extend(true, {}, DataTable.defaults.altEditor, altEditor.defaults,
                    opts);
  ...
  this._constructor();
    ...
    $.extend(altEditor.prototype, {
      ...
      _constructor : function() {
        ...
        // Load from URL
        else if (typeof lang.altEditorUrl === 'string' &&
                 lang.altEditorUrl != '') {
          $.ajax({
            dataType : 'json',
            ...
            error : function() {
              // Error occurred loading language file, continue on as best we
              // can
              that.language = {};
              that._setup();
            }
      ...
      _setup : function() {
        ...
        var modal =
            '<div class="modal fade altEditor-modal reveal" id="' + modal_id +
        ...
        $(this.modal_selector).on('input', '[data-unique]', function(elm) {
          if ($(elm.target).attr('data-unique') == null ||
              $(elm.target).attr('data-unique') === 'false') {
            return;
          }
          var target = $(elm.target);
          var colData =
              dt.column("th:contains('" + target.attr("name") + "')").data();
          // go through each item in this column
          var selectedCellData = null;
          if (dt.row({selected : true}).index() != null)
            selectedCellData =
                dt.cell(dt.row({selected : true}).index(),
                        dt.column("th:contains('" + target.attr("name") + "')")
                            .index())
                    .data();
          elm.target.setCustomValidity('');
          for (var j in colData) {
            // if the element is in the column and its not the selected one then
            // its not unique
      ...
      _initLanguage : function() {
        ...
        this.language.error = {
          message :
              this.language.error.message || 'There was an unknown error!',
      ...
      _emitEvent : function(name, args) {
        this.s.dt.iterator(
            'table',
            function(ctx,
                     i) { $(ctx.nTable).triggerHandler(name + '.dt', args); });
      },
      ...
      _openEditModal : function() {
        ...
        var data = this.createDialog(
            columnDefs, this.language.edit.title, this.language.edit.button,
            ...
            // Added checkbox
              ...
              if (this._quoteattr(selectedValue) === "true" ||
                  this._quoteattr(selectedValue) ==
                      "1") { // MS SQL Databases use bits for booleans. 1 is
                             // equivlent to true, 0 is false
                $(selector)
                    .find(jquerySelector)
                    .prop(
                        "checked",
                        this._quoteattr(selectedValue)); // required by checkbox
              }
            }
            // Added date
      ...
      _editRowData : function() {
        ...
        $inputs.filter(':input[type!="file"], :input[type="checkbox"]')
            .each(function(
                i) { rowDataArray[$(this).attr('id')] = $(this).val(); });
        ...
        var checkFilesQueued = function() {
          if (numFilesQueued == 0) {
            that.onEditRow(
                that, rowDataArray,
                function(data, b, c, d,
                         e) { that._editRowCallback(data, b, c, d, e); },
      ...
      _openDeleteModal : function() {
        ...
        var fill = function() {
          ...
          $(selector)
              .find('.modal-body')
              .html(that.language.deleteMessage ||
                    `<h5>Are you sure you wish to delete ${
                        adata.count()} rows?</h5>`);
          ...
          if (modalContent.parent().is('form')) {
            ...
            modalContent.wrap("<form name='" + formName + "' id='" + formName +
                              "' role='form'></form>");
          }
        };
      ...
      _openAddModal : function() {
        ...
        var columnDefs = this.completeColumnDefs();
        var data = this.createDialog(
            columnDefs, this.language.add.title, this.language.add.button,
      ...
      completeColumnDefs : function() {
        ...
        for (var i in dt.context[0].aoColumns) {
          ...
          columnDefs[i] = {
            ...
            optionsSortByLabel :
                (obj.optionsSortByLabel ? obj.optionsSortByLabel : false),
            inline :
                (obj.inline ? obj.inline : false), // Added for inline columns
       ...
       */
      createDialog : function(columnDefs, modalTitle, buttonCaption,
                              closeCaption, buttonClass, formName) {
        formName = [ formName, this.random_id ].join('-');
        var that = this, data = "", count = 0;
        ...
        for (var j in columnDefs) {
          ...
          if (columnDefs[j].type.indexOf("hidden") >= 0) {
            data += "<input type='hidden' " +
                    ...
                    fillAttrs(columnDefs[j], [ 'name', 'value' ]) + "></input>";
          } else {
            ...
            if (columnDefs[j].inline) { // to add upto 4 inline columns
              if (count == 0) {
                ...
                data += "<label for='" + this._quoteattr(columnDefs[j].name) +
                        "'>" + title + ":</label></div>";
            ...
            if (columnDefs[j].type.indexOf("select") >= 0) {
              var options = "", optionsArray = columnDefs[j].options;
            ...
            // Adding Text Area
              ...
              data += "<textarea class='form-control' " +
                      ...
                      fillAttrs(columnDefs[j],
                                [
                                  'name', 'style', 'rows', 'cols', 'maxLength',
            ...
            // Adding text-inputs and error labels, but also new HTML5 types
            // (email, color, ...)
            else {
              data += "<input class='form-control' " +
                      fillAttrs(columnDefs[j],
                                [
                                  'type', 'pattern', 'accept', 'name', 'step',
                                  'min', 'max', 'maxLength', 'value',
                                  'readonly', 'disabled', 'required'
                                ]) +
                      /* ???? */
            ...
            data += "<label id='" + this._quoteattr(columnDefs[j].name) +
                    "-label" +
        ...
        for (var j in columnDefs) {
          ...
          if (columnDefs[j].editorOnChange) {
            ...
            $(selector)
                ...
                .on('change', function(elm) {
                  var f =
                      columnDefs[$(this).attr('alt-editor-id')].editorOnChange;
                  f(elm, that);
                });
          }
          ...
          if (columnDefs[j].type.indexOf("select") >= 0) {
            if (columnDefs[j].optionsSortByLabel) {
              ...
              opts_list.sort(function(
                  a, b) { return $(a).text() > $(b).text() ? 1 : -1; });
      ...
      onDeleteRow : function(dt, rowdata, success, error) {
        ...
        rowdata.every(function(rowIdx, tableLoop,
                               rowLoop) { success(rowdata[rowIdx]) })
      },
      ...
      internalOpenDialog(selector, onopen) {
        ...
        if ($sel.modal) {
          ...
          $sel.on('closed.zf.reveal',
                  function() { $('.reveal-overlay').hide(); });
      ...
      _quoteattr : function(s, preserveCR) {
        ...
        return ('' + s)             /* Forces the conversion to string. */
            ...
            .replace(/>/g, '&gt;')
            .replace(/\r\n/g,
                     preserveCR) /* Must be before the next replacement. */
            .replace(/[\r\n]/g, preserveCR);
      },
    });
