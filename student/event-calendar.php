...
<body>
  <div class="d-flex" id="wrapper">
    ...
    <div id="content">
      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item"><a href="rso.php"> <i class="bi bi-people-fill"></i> <?php $query = "SELECT * FROM tb_orgs WHERE ORG_ID = '$morg_id'";
                  $result = @mysqli_query($conn, $query);
                  $row = mysqli_fetch_array ($result);
                  if ($row)
                  { echo "$row[1]"; }
    ...
    <?php
    $schedules = $conn->query("SELECT project_id,project_name,start_date, end_date  FROM `tb_projectmonitoring` WHERE org_id='$morg_id' AND status='Approved'");
    $sched_res = [];
