                ...
                <div class="card-body mt-2 pt-0">
                  <table class="table table-bordered" id="proftable">
                    ...
                    <tr>
                      ...
                      <td width="2%">:</td>
                      <td><?php $query = "SELECT tb_signatories.signatorytype_id, tb_signatory_type.signatory FROM tb_signatories INNER JOIN tb_signatory_type ON tb_signatories.signatorytype_id=tb_signatory_type.signatory_id WHERE tb_signatories.school_id = '$data_userid'";
                          $result = @mysqli_query($conn, $query);
                          $row = @mysqli_fetch_array($result);
                          if ($row) {
                            echo "$row[signatory]";
                    ...
                    <tr>
                      <th width="30%">Role: </th>
                      <td width="2%">:</td>
                      <td><?php $query = "SELECT tb_signatories.usertype_id, tb_usertypes.user_type FROM tb_signatories INNER JOIN tb_usertypes ON tb_signatories.usertype_id=tb_usertypes.usertype_id WHERE tb_signatories.school_id = '$data_userid'";
                          $result = @mysqli_query($conn, $query);
                          $row = @mysqli_fetch_array($result);
                          if ($row) {
                            echo "$row[user_type]";
                  ...
                  <div class="card-body mt-2 p-0 w-100 pt-0" id="card-show">
                    ...
                    <p class="mb-2 pe-2"><strong class="pr-1  text-muted">Signatory Type:<br></strong><?php $query = "SELECT tb_signatories.signatorytype_id, tb_signatory_type.signatory FROM tb_signatories INNER JOIN tb_signatory_type ON tb_signatories.signatorytype_id=tb_signatory_type.signatory_id WHERE tb_signatories.school_id = '$data_userid'";
                                                                                                      $result = @mysqli_query($conn, $query);
                                                                                                      $row = @mysqli_fetch_array($result);
                                                                                                      if ($row) {
                                                                                                        echo "$row[signatory]";
                                                                                                      } ?></p>
                    <p class="mb-2 pe-2"><strong class="pr-1  text-muted">College:<br></strong><?php $query = "SELECT tb_signatories.college_dept, tb_collegedept.college FROM tb_signatories INNER JOIN tb_collegedept ON tb_signatories.college_dept=tb_collegedept.college_id WHERE tb_signatories.school_id = '$data_userid'";
                                                                                                $result = @mysqli_query($conn, $query);
                                                                                                $row = @mysqli_fetch_array($result);
                                                                                                if ($row) {
                                                                                                  echo "$row[college]";
                                                                                                } ?></p>
                    <p class="mb-2 pe-2"><strong class="pr-1  text-muted">Organization:<br></strong><?php $query = "SELECT tb_signatories.org_id, tb_orgs.ORG FROM tb_signatories INNER JOIN tb_orgs ON tb_signatories.org_id=tb_orgs.ORG_id WHERE tb_signatories.school_id = '$data_userid'";
                                                                                                    $result = @mysqli_query($conn, $query);
                                                                                                    $row = @mysqli_fetch_array($result);
                                                                                                    if ($row) {
                                                                                                      echo "$row[ORG]";
